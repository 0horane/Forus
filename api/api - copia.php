...
//yr: NO IMPLEMENTADO funciona igual que sr, pero busca las recetas del usuario ordenadas. Devuelve una lista con als recetas sin borrar y otra con borradas
//ys: NO IMPLEMENTADO funciona igual que sr, pero busca las recetas guardadas del usuario ordenadas. Tambien permite usar el valor 'm' como primera letra para ver mas recientes
//sf: NO IMPLEMENTADO cambia el estado de favorito de una receta. Devuelve True si queda en favoritos y False si queda no en favorito.
...
function orderAndPush($query,$link,$order='a'){
    switch ($order){
...
switch($qt){
    ...
    case 'sr':
        ...
        switch ($orderarr[0]){
            ...
            case 'p': $query="SELECT recipes.*, COUNT(favorites.Recipes_ID) as popularity FROM recipes LEFT JOIN favorites ON favorites.Recipes_ID = recipes.ID AND favorites.Created_At + INTERVAL 7 DAY > NOW() WHERE Deleted_At IS NULL GROUP BY recipes.ID ORDER BY popularity"; break;
        }
        $json=orderAndPush($query,$link,$orderarr[1]);
        break;
